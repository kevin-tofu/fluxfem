...
def parse_args():
    ...
    p.add_argument("--output-vtu", default="", help="Optional VTU path for deformed plate visualization.")
...
def main():
    ...
    if args.output_vtu:
        ...
        coords3[:, :2] = coords
        disp = np.zeros_like(coords3)
        disp[:, 2] = w_nodes
        rotations = np.column_stack([np.asarray(u)[1::3], np.asarray(u)[2::3], np.zeros(coords.shape[0])])
