...
class MortarContactProblemPair:
    ...
    name: str
    operators: MultiplierContactContribution
    ...
    def diagnostics(self, **kwargs) -> dict[str, Any]:
        ...
        return {
            ...
            "coupling_nnz": int(_contact_matrix_nnz(self.operators.B)),
...
class MortarContactProblem:
    ...
    @property
    def matrix(self):
        return _contact_kkt_matrix_from_blocks(self.stiffness, self.coupling_matrix)
    ...
    def solve_with_info(self, config: "ContactKKTSolveConfig | None" = None) -> "ContactKKTSolveResult":
        cfg = ContactKKTSolveConfig(backend="numpy", numpy_solver="block_scaled", n_primal=int(self.stiffness.shape[0]))
        if config is not None:
            cfg = replace(config, n_primal=int(self.stiffness.shape[0]) if config.n_primal is None else config.n_primal)
...
def _contact_matrix_nnz(matrix: Any) -> int:
    if matrix is None:
        return 0
...
def _contact_to_csr_matrix(matrix: Any):
    try:
        import scipy.sparse as sp
    except Exception:  # pragma: no cover
        return np.asarray(_dense_contact_operator_matrix(matrix, backend="numpy"), dtype=float)
...
def _contact_vstack(matrices: Sequence[Any]):
    try:
        import scipy.sparse as sp
    except Exception:  # pragma: no cover
        return np.vstack([np.asarray(mat, dtype=float) for mat in matrices]) if matrices else np.zeros((0, 0))
    return sp.vstack([_contact_to_csr_matrix(mat) for mat in matrices], format="csr")


def _contact_kkt_matrix_from_blocks(stiffness: Any, coupling_matrix: Any):
    try:
        import scipy.sparse as sp
    except Exception:  # pragma: no cover
    ...
    K = _contact_to_csr_matrix(stiffness)
    B = _contact_to_csr_matrix(coupling_matrix)
    zero = sp.csr_matrix((B.shape[0], B.shape[0]), dtype=K.dtype)
    return sp.bmat([[K, B.T], [B, zero]], format="csr")


def _embed_contact_pair_B(
    ops: MultiplierContactContribution,
    *,
    ...
    B_local = _contact_to_csr_matrix(ops.B)
    ...
    *,
    ...
    for idx, pair in enumerate(contact_pairs):
        if isinstance(pair, MortarContactProblemPair):
            ...
            if not isinstance(ops, MultiplierContactContribution):
                raise TypeError("contact_pairs entries require a MultiplierContactContribution under 'operators' or 'ops'.")
            master = pair["master_dofs"]
            slave = pair["slave_dofs"]
            name = str(pair.get("name", f"contact-{idx}"))
        block = _embed_contact_pair_B(
            ops,
            master_dofs=master,
            slave_dofs=slave,
            n_primal=n_primal,
    ...
    if embedded_blocks:
        B_global = _contact_vstack(embedded_blocks)
    else:
        try:
            import scipy.sparse as sp
        except Exception:  # pragma: no cover
    ...
    return MortarContactProblem(
        stiffness=_contact_to_csr_matrix(stiffness),
