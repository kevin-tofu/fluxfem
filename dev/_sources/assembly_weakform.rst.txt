...
- ``ddot(a, b)`` computes elementwise double contraction
  ``sum_ij a_ij b_ij`` for 2D arrays, returning a scalar per quadrature point.
- ``ddot(a, b)`` for 3D arrays shaped like ``(q, i, k)`` and ``(q, i, m)``
  contracts over the middle axis and returns a local block ``(q, k, m)``
  (used by ``sym_grad`` in elasticity).
- ``ddot(a, b, c)`` computes ``a^T b c`` (used for Voigt-style elasticity blocks).
