...
- Common mortar choices have factory constructors: `MultiplierSpec.dual_mortar()`, `MultiplierSpec.coarse_dual_mortar()` for auto SVD/QR row reduction, `MultiplierSpec.nodal_mortar()`, and `MultiplierSpec.p0_mortar(contact)`.
...
- `tutorials/craig_bampton_multifield_builder.py` extends the same builder pattern to multiple reduced subsystems connected by named retained interface groups, including surface-derived groups and contact-pair parameter metadata for contact-ready interfaces.
