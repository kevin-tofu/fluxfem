...
quadrature update helpers are available. The J2 load-step helper commits
quadrature histories after converged steps, supports displacement and external
load scaling, and exposes per-step Newton iteration diagnostics for convergence
checks. Quadrature strain/stress diagnostics are exposed for verification, with
tests covering material-point references, unload/reload elastic increments,
force-controlled elastic loading, mixed-BC plastic loading, and small multi-hex
step-refinement consistency. `tutorials/nonlinear/j2_uniaxial_tension.py`
writes VTU cell data, a CSV load history, and a Newton iteration CSV for full
displacement, mixed-BC, or small force-controlled J2 bar examples. Production
restart/output and general lifecycle management for plasticity, continuum
viscoelasticity, and damage are not mature yet; see
...
<ul>
  ...
  <li><strong>Materials and multiphysics</strong>
    <ul>
      <li>Linear elasticity, diffusion/heat examples, thermoelastic examples, Neo-Hookean hyperelastic residual examples, and J2 material-point / FE quadrature-state / basic load-step entry points are available.</li>
      <li>Production restart/output and general lifecycle management for plasticity, general viscoelasticity, and damage is not yet mature.</li>
    </ul>
  </li>
