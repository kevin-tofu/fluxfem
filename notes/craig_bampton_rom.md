...
- A combined solid + shell + beam model now has static, constrained modal, and
  constrained Newmark full-KKT vs CB-projected-KKT regressions. The same solid
  body carries a translational shell skin tie and an RBE3 beam interface, and
  the combined structural block is reduced as one CB subsystem.
