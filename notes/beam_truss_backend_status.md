...
- The solid-beam and solid-truss coupling tests also project the mixed
  ...
  and constrained modal frequencies, plus constrained Newmark displacement
  histories from an initial low-order constrained mode. RBE3 source-copy and
  remote-point auxiliary DOFs remain explicit and massless.
...
- `add_distributed_coupling(...)`: appends an auxiliary patch copy, appends a
  ...
  when the patch should not reconstruct remote rotations; the reduced component
  system is assembled without remote-rotation columns.
...
- `tutorials/contact/contact_supported_box_by_truss_springs.py`
